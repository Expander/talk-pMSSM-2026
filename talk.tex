...
\begin{frame}{Fixed-order calculation (\texttt{NUHMSSMNoFVHimalaya})}
  \begin{center}
    \begin{tabular}{llll}
      ...
      \midrule
      $y_t$      & full & \textcolor{darkgreen}{$y_tg_3^4$} & -- \\
      $y_{b,\tau}$ & full & -- & -- \\
      $g_3$      & full & $g_3^4$ & -- \\
      $g_{1,2}$   & full & -- & -- \\
      $v$        & full & -- & -- \\
      ...
      $\beta_{v_i}$  & full & full & -- \\
      \midrule
      $M_h$  & full & $g_3^2(y_t^4+y_b^4)+(y_t^2+y_b^2)^3+(y_t^2+y_\tau^2)^3$ & \textcolor{darkgreen}{$g_3^4 y_t^4$} \\
      \bottomrule
    \end{tabular}
  \end{center}
  \textcolor{darkgreen}{green}: required for N$^3$LO SQCD precision
\end{frame}
...
\begin{frame}{Pure EFT calculation (\texttt{HSSUSY})}
  \begin{center}
    ...
    \begin{tabular}{lllll}
      ...
      \midrule
      $\hat\lambda$  & full & $(\hat{y}_t^4+\hat{y}_b^4)\hat{g}_3^2+(\hat{y}_t^4+\hat{y}_b^4+\hat{y}_\tau^4)^2$ & \textcolor{darkgreen}{$\hat{y}_t^4\hat{g}_3^4$} & -- \\
      $\hat{v}$        & full & -- & -- & -- \\
      $\hat{y}_t$      & full & $\textcolor{darkgreen}{\hat{y}_t\hat{g}_3^4}{}+\hat{y}_t^3 \hat{g}_3^2+\hat{y}_t^5$ & $\hat{y}_t\hat{g}_3^6$ & $\hat{y}_t\hat{g}_3^8$ \\
      $\hat{y}_{b,\tau}$ & full & -- & -- & -- \\
      $\hat{g}_3$      & full & $\hat{g}_3^4$ & $\hat{g}_3^6$ & -- \\
      $\hat{g}_{1,2}$   & full & -- & -- & -- \\
      \midrule
      $\beta_{\hat\lambda}$ & full & full & full & \textcolor{darkgreen}{$\hat{y}_t^4\hat{g}_3^6$} \\
      ...
      \midrule
      $M_h$           & full & $(\hat{y}_t^4+\hat{y}_b^4)\hat{g}_3^2+(\hat{y}_t^4 + \hat{y}_b^4)^2$ & $\textcolor{darkgreen}{\hat{y}_t^4 \hat{g}_3^4}{}+\hat{y}_t^6\hat{g}_3^2+\hat{y}_t^8$ & $\hat{y}_t^4 \hat{g}_3^6$ \\
      \bottomrule
    \end{tabular}}
  \end{center}
  \textcolor{darkgreen}{green}: required for N$^3$LO+N$^3$LL SQCD precision
\end{frame}
...
\begin{frame}{Hybrid calculation (FlexibleEFTHiggs) \mycite{1609.00371, 2003.04639, 2506.22208}} % (\texttt{NUHMSSMNoFVHimalayaEFTHiggs})
  ...
  \begin{align*}
    ...
    \Rightarrow\qquad \hat{\lambda}(\Qmatch)
  ...
  \emph{Precision:} N$^3$LO+N$^3$LL (same as pure EFT calculation)
  \begin{itemize}
  \item includes terms of $\ord(v^n/\MS^n)$
    due to pole mass matching
    % at $1\Loop$ and $2\Loop$ % at the respective orders
  \item resummation of subleading logs due to correct combination with
    threshold corrections (no double counting)
  \item resummation of QCD-enhanced terms of $\ord(x_t^n)$ due to
...
\begin{frame}{}
  \begin{center}
    ...
    \includegraphics[width=5cm]{images/FS-logo.png}
  \end{center}
\end{frame}
...
\newcommand{\FOMSdata}{data/NUHMSSMNoFVHimalaya_MS-Mh-DMh_TB-20_xt--2.44949.dat}
\newcommand{\FOXtdata}{data/NUHMSSMNoFVHimalaya_Xt-Mh-DMh_MS-3000_TB-20.dat}
\newcommand{\EFTMSdata}{data/HSSUSY_MS-Mh-DMh_TB-20_xt--2.44949.dat}
\newcommand{\EFTXtdata}{data/HSSUSY_Xt-Mh-DMh_MS-3000_TB-20.dat}
\newcommand{\FEFTMSdata}{data/NUHMSSMNoFVHimalayaEFTHiggs_MS-Mh-DMh_TB-20_xt--2.44949.dat}
\newcommand{\FEFTXtdata}{data/NUHMSSMNoFVHimalayaEFTHiggs_Xt-Mh-DMh_MS-3000_TB-20.dat}

\begin{frame}{Comparison of FO, EFT, Hybrid}
  \begin{center}
    \begin{tikzpicture}
      \begin{groupplot}[
        group style={
        ...
        \addplot[black,thick,densely dotted] table[x index=0, y index=1] {\FOMSdata};
        \addlegendentry{FO};
        \addplot[blue,thick,densely dashed] table[x index=0, y index=1] {\EFTMSdata};
        \addlegendentry{EFT};
        \addplot[red,thick] table[x index=0, y index=1] {\FEFTMSdata};
        \addlegendentry{Hybrid};
        %
        \addplot[name path=upper,draw=none] table[x index=0, y expr=\thisrowno{1}+\thisrowno{2}] {\FOMSdata};
        \addplot[name path=lower,draw=none] table[x index=0, y expr=\thisrowno{1}-\thisrowno{2}] {\FOMSdata};
        \addplot[black!20,fill opacity=0.5] fill between[of=upper and lower];
        %
        \addplot[name path=upper,draw=none] table[x index=0, y expr=\thisrowno{1}+\thisrowno{2}] {\EFTMSdata};
        \addplot[name path=lower,draw=none] table[x index=0, y expr=\thisrowno{1}-\thisrowno{2}] {\EFTMSdata};
        \addplot[blue!40,fill opacity=0.5] fill between[of=upper and lower];
        %
        \addplot[name path=upper,draw=none] table[x index=0, y expr=\thisrowno{1}+\thisrowno{2}] {\FEFTMSdata};
        \addplot[name path=lower,draw=none] table[x index=0, y expr=\thisrowno{1}-\thisrowno{2}] {\FEFTMSdata};
        \addplot[red!40,fill opacity=0.5] fill between[of=upper and lower];
        %%
        \nextgroupplot[height=3cm,ylabel={$\Delta M_h$/GeV},xlabel={$\MS$/GeV},ymin=0,ymax=4,ytick distance=1]
        \addplot[black,thick,densely dotted] table[x index=0,y index=2] {\FOMSdata};
        \addplot[blue,thick,densely dashed] table[x index=0,y index=2] {\EFTMSdata};
        \addplot[red,thick] table[x index=0,y index=2] {\FEFTMSdata};
      \end{groupplot}
    \end{tikzpicture}
  \end{center}
\end{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      \begin{groupplot}[
        ...
        \addplot[black,thick,densely dotted,restrict x to domain=-3.5:1] table[x index=0, y index=1] {\FOXtdata};
        \addlegendentry{FO};
        \addplot[blue,thick,densely dashed] table[x index=0, y index=1] {\EFTXtdata};
        \addlegendentry{EFT};
        \addplot[red,thick] table[x index=0, y index=1] {\FEFTXtdata};
        \addlegendentry{Hybrid};
        %
        \addplot[black,thick,densely dotted,restrict x to domain=1:3.5] table[x index=0, y index=1] {\FOXtdata};
        \addplot[name path=upper,draw=none,restrict x to domain=-3.5:1] table[x index=0, y expr=\thisrowno{1}+\thisrowno{2}] {\FOXtdata};
        \addplot[name path=lower,draw=none,restrict x to domain=-3.5:1] table[x index=0, y expr=\thisrowno{1}-\thisrowno{2}] {\FOXtdata};
        \addplot[black!20,fill opacity=0.5] fill between[of=upper and lower];
        %
        \addplot[name path=upper,draw=none] table[x index=0, y expr=\thisrowno{1}+\thisrowno{2}] {\EFTXtdata};
        \addplot[name path=lower,draw=none] table[x index=0, y expr=\thisrowno{1}-\thisrowno{2}] {\EFTXtdata};
        \addplot[blue!40,fill opacity=0.5] fill between[of=upper and lower];
        %
        \addplot[name path=upper,draw=none] table[x index=0, y expr=\thisrowno{1}+\thisrowno{2}] {\FEFTXtdata};
        \addplot[name path=lower,draw=none] table[x index=0, y expr=\thisrowno{1}-\thisrowno{2}] {\FEFTXtdata};
        \addplot[red!40,fill opacity=0.5] fill between[of=upper and lower];
        %%
        \nextgroupplot[height=3cm,ylabel={$\Delta M_h$/GeV},xlabel={$x_t$ },ymin=0,ymax=3,ytick distance=1]
        \addplot[black,densely dotted,thick,restrict x to domain=-3.5:1] table[x index=0,y index=2] {\FOXtdata};
        \addplot[blue,thick,densely dashed] table[x index=0,y index=2] {\EFTXtdata};
        \addplot[red,thick] table[x index=0,y index=2] {\FEFTXtdata};
      \end{groupplot}
    \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{Uncertainty estimate of the fixed-order calculation}
  Possible uncertainty estimates at $3\Loop$ precision:
  \begin{align*}
...
\begin{frame}{Uncertainty estimate of the hybrid calculation}
  Possible uncertainty estimates at N$^3$LO+N$^3$LL:
  \begin{align*}
    \DMhQpole &= \max_{\Qpole\in[M_t/2,2M_t]}\left|M_h(\Qpole) - M_h(M_t)\right| \\
    \DMhQmatch &= \max_{\Qmatch\in[\MS/2,2\MS]}\left|M_h(\Qmatch) - M_h(\MS)\right| \\
    \DMhQlow &= \max_{\Qlow\in[M_Z/2,2M_Z]}\left|M_h(\Qlow) - M_h(M_Z)\right| \\
