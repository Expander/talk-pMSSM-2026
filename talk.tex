...
\begin{frame}{Fixed-order calculation}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.9]
      ...
      \draw[thick] (0,1) node[left]{$\Qlow=M_Z$} -- node[above=1cm,black]{MSSM} node[below=0.5cm,black]{SM(5)} ++ (\breite,0) node[right]{calculate $g_i, y_f$, $v$};
    \end{tikzpicture}
  \end{center}
\end{frame}
...
\begin{frame}{Pure EFT calculation (\texttt{HSSUSY})}
  ...
  \begin{tikzpicture}
    \pgfmathsetmacro{\breite}{2};
    \draw[->,thick] (0,0) -- (0,5) node[above]{$Q$};
    \draw[thick] (0,4) node[left]{$\Qmatch = \MS$} -- node[above = 0.5cm,black]{MSSM} (\breite,4) node[right,black]{calculate $\hat\lambda(\hat y_f, \hat g_i, m_{\tilde{f}}^2, A_f, M_i,\dotsc)$};
    ...
    \draw[thick] (0,1) node[left]{$\Qlow = M_Z$} -- node[above = 1.5cm,black]{SM} node[below = 0.5cm,black]{SM(5)} (\breite,1) node[right,black,text width=8cm]{
