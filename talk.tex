...
\begin{frame}{Pure Effective Field Theory calculation}
  \emph{Input:} same as in fixed-order calculation
  \begin{center}
    \begin{tikzpicture}[scale=0.9]
      \draw[->, thick] (0,0) -- (0,6) node[above] {$Q$};
      \draw[thick] (0.1,1) -- ++ (-0.2,0) node[left]{$Q_{\text{low}}\approx M_t$};
      \draw[thick] (0.1,5) -- ++ (-0.2,0) node[left]{$Q_{\text{match}}\approx\sqrt{m_{\tilde{t}_1}m_{\tilde{t}_2}}$};
      \draw[thick] (1,0)   rectangle node{SM(5)} (4,0.9);
      \draw[thick] (1,1.1) rectangle node{SM}    (4,4.9);
      \draw[thick] (1,5.1) rectangle node{MSSM}  (4,6);
      \draw[<-, thick, darkgreen] (4.1,5) -- (4.5,5) node[right]{calculate $\hat{\lambda}(Q_{\text{match}})$};
      \draw[<-, thick, darkgreen] (4.1,1) -- (4.5,1) node[right,align=left]{fix $\hat{g}_i, \hat{y}_f$, $\hat{v}$\\ \textcolor{red}{output $M_h^\SM$}};
      \draw[<->, thick, blue] (4.2,1.2) -- node[right, align=center]{RG running} (4.2,4.8);
    \end{tikzpicture}
  \end{center}
\end{frame}
