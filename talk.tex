...
\date{pMSSM topic meeting\\[1em] ??/??/2026}
...
\begin{frame}[plain]
  ...
  \node[anchor=south east] at
    ([yshift=0.5cm,xshift=-0.5cm]current page.south east)
    {\includegraphics[width=4cm]{images/RWTH_Logo.pdf}};
  \titlepage
\end{frame}
...
\begin{frame}{FlexibleSUSY}
  \begin{center}
    \begin{tikzpicture}
      \node[inner sep=0pt] (FS) at (0,0) {\includegraphics[width=.8\textwidth]{images/FS.png}};
      \draw[latex-,thick] ([xshift=-3cm,yshift=0cm] FS.east) to[out=0,in=270] ++(2cm,1cm) node [above,yshift=0.5em] {\textbf{FlexibleSUSY}};
    \end{tikzpicture}
  \end{center}
  ...
  Download:
  \href{https://flexiblesusy.hepforge.org}{flexiblesusy.hepforge.org},
...
\begin{frame}{Fixed-order calculation}
  ...
  \begin{center}
    \begin{tikzpicture}[scale=0.9]
      \draw[->, thick] (0,0) -- (0,1) node[left]{$M_t$} -- (0,4.5) node[left]{$\MS$} -- (0,5.5) node[left]{$Q_\text{input}$} -- (0,6) node[above] {$Q$};
      ...
      \draw[<-, thick, darkgreen] (4.1,1) -- (4.5,1) node[right]{fix $y_i^\MSSM, y_f^\MSSM$, $v^\BSM$};
...
\subsection{Pure Effective Field Theory}
