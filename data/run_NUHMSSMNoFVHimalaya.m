...
scanMS = ParallelMap[{N[#], Sequence @@ NUHMSSMNoFVHimalayaCalcMhDMh[#, TBX, Xtt]}&, msValues];

Export["NUHMSSMNoFVHimalaya_MS-Mh-DMh_TB-" <> ToString[TBX] <> "_xt-" <> ToString[N[Xtt]] <> ".dat", scanMS];

(* scan over xt *)

xtValues = Subdivide[-3.5, 3.5, 60];

scanXt = ParallelMap[{N[#], Sequence @@ NUHMSSMNoFVHimalayaCalcMhDMh[MSX, TBX, #]}&, xtValues];

Export["NUHMSSMNoFVHimalaya_Xt-Mh-DMh_MS-" <> ToString[MSX] <> "_TB-" <> ToString[TBX] <> ".dat", scanXt];
